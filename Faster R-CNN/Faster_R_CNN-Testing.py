...
# get image from testing dataset - selecting a random image 
testing_dir = './trashy-dataset-roboflow.coco/test'
# sample random image
img = random.choice(os.listdir(testing_dir))
...
def draw_bboxes(output_dir, image, prediction, fig_size):
...
draw_bboxes(output_dir, Image.open(img_path), prediction, fig_size=(8,8))
# Close all figures
plt.close('all')
